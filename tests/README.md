...
Version 0.3 uses dependency-free, read-only Python conformance checks. Python 3.10 or
...
## Other 0.3 semantic suites

- `validate_context_authority_v030.py`: protocol/core-lock identity, maturity/effect
  containment, context delivery, and bounded continuation.
- `validate_protocol_security_v030.py`: typed references, exact subjects, evidence and
  review authenticity, and malicious substitutions.
- `validate_adoption_v030.py`: qualification, resource, adoption-efficacy, and policy
  semantics.
- `validate_task_rigor_v030.py`: advisory task-rigor selection and proportional
  evidence/scaffolding rules.
- `validate_release_bundle_v030.py`: exact authenticated Level 0 artifact inventory.
