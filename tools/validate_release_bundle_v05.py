#!/usr/bin/env python3
"""Validate the exact inert ELAD 0.5 normative protocol bundle."""
...
def main() -> int:
    try:
        ...
        deny(bundle.get("schemaVersion") == bundle.get("version") == "0.5", "bundle version mismatch")
        deny(bundle.get("bundleId") == "protocol-bundle:elad_0.5", "bundle identity mismatch")
        deny(bundle.get("protocolId") == blueprint.get("protocolId") == "protocol:elad_0.5", "protocol identity mismatch")
        ...
        print(f"PASS — ELAD 0.5 release bundle authenticates {len(entries)} normative artifacts; Level-0 reference only, no self-entry, no authority granted.")
        return 0
