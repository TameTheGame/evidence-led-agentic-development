...
0.3.0 context/authority/core-lock correction.  It uses an in-memory synthetic fixture;
...
def validate_maturity_registry(vectors: dict[str, Any]) -> dict[int, dict[str, Any]]:
    ...
    require(registry.get("schemaVersion") == "0.3.0", "maturity registry version mismatch")
...
def validate_protocol_bundle(bundle: dict[str, Any], files: dict[str, bytes], expected_paths: set[str]) -> None:
    require(bundle.get("schemaVersion") == bundle.get("version") == "0.3.0", "protocol bundle version mismatch")
    require(bundle.get("bundleId") == "protocol-bundle:elad_0.3.0", "protocol bundle ID mismatch")
    require(bundle.get("protocolId") == "protocol:elad_0.3.0", "protocol ID mismatch")
...
def validate_reference(reference: dict[str, Any], identifier: str, path: str, document: dict[str, Any], label: str) -> None:
    ...
    require(reference.get("schemaVersion") == "0.3.0", f"{label} schema version mismatch")
...
def validate_core_lock(
    ...
    require(blueprint.get("version") == bundle.get("version") == "0.3.0", "core lock blueprint version mismatch")
...
def build_fixture() -> dict[str, Any]:
    protocol_files = {
        "blueprint.json": canonical_json_bytes({"schemaVersion": "0.3.0", "protocolId": "protocol:elad_0.3.0"}),
        "VERSION": b"0.3.0\n",
        "spec/schemas/synthetic.schema.json": canonical_json_bytes({"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object"}),
        "spec/registries/synthetic.json": canonical_json_bytes({"schemaVersion": "0.3.0", "values": ["synthetic"]}),
        "tests/synthetic-oracle.json": canonical_json_bytes({"schemaVersion": "0.3.0", "expected": "deny_by_default"}),
    }
    bundle = {
        "schemaVersion": "0.3.0",
        "bundleId": "protocol-bundle:elad_0.3.0",
        "protocolId": "protocol:elad_0.3.0",
        "version": "0.3.0",
...
def apply_mutation(fixture: dict[str, Any], mutation: str) -> None:
    ...
    if mutation == "valid_need_context":
        ...
        fixture["core"]["protocolBundle"]["id"] = "protocol:elad_0.3.0"
...
def main() -> int:
    ...
    require(vectors.get("schemaVersion") == "0.3.0", "vector version mismatch")
    ...
    print(
        "PASS — protocol 0.3 context/authority slice: "
